...
repeat = 1000
epochs = 1001
file_path = "data/lotr_clean.txt"
load_path = "model_save/char.chk"
save_path = "model_save/char.chk"
max_words = 8001
save_epoch = 500
lamb = 0.3
predict_epoch = 5
...
ids_to_words = np.array(list(set(file_contents)))
words_to_ids = { c: i for (i, c) in enumerate(ids_to_words) }
num_ids = len(ids_to_words)

all_words = np.array([ words_to_ids[c] for c in file_contents ])
