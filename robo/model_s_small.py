...
dt_hidden_size = 16
max_dt = 0.02
initial_trans_box = [[ -4.0, 4.0 ], [ -4.0, 0.0 ]]
...
lamb_len = 0.0
lamb_fast = 0.0
load_path = "model_save/model_novel_small2"
save_path = "model_save/model_novel_small2"
# THIS IS GOOD
# save_path = "model_save/model_novel_small2"
log_path = "model_log/"
...
def dt_model(X):
    ...
    return tf.maximum(0.5 * (max_dt + 0.1) * (out + 1.0) - 0.1, 0.0)

def update(trans, vel, dt, d):
    vel_t = tf.transpose(vel)
    lv = vel_t[1] * (vel_t[0] * (max_vel - min_vel) + min_vel)
    rv = (1.0 - vel_t[1]) * (vel_t[0] * (max_vel - min_vel) + min_vel)
    dtheta = -dt * (rv - lv) / d
...
initial_trans = tf.random_uniform([ batch_size, 2 ])
...
initial_trans = tf.concat([ initial_trans, tf.expand_dims(tf.atan(initial_trans[:,0] / initial_trans[:,1]), axis = 1) ], axis = 1)
